...
I also created a simple GitOps workflow by just running a systemd timer that frequently pulls from this repo and refreshes systemd. The script checks the git diff-tree and gets a list of .container files that are different from last commit. It then starts, restarts, or deletes systemd service based on the status.
